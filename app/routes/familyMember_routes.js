...
// INDEX
// GET /examples
router.get('/familyMembers', requireToken, (req, res, next) => {
  const owner = req.user.id
  FamilyMember.find({ owner: owner })
    .then(familyMembers => {
      // `examples` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return familyMembers.map(familyMember => familyMember.toObject())
    })
...
router.get('/familyMembers/:id', requireToken, (req, res, next) => {
...
// CREATE
// POST /examples
router.post('/familyMembers', requireToken, (req, res, next) => {
...
router.patch('/familyMembers/:id', requireToken, removeBlanks, (req, res, next) => {
...
router.delete('/familyMembers/:id', requireToken, (req, res, next) => {
  FamilyMember.findById(req.params.id)
    .then(handle404)
    .then(familyMember => {
